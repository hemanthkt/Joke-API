...
app.get("/filter", (req, res)=> {
  ...
  const filteredJoke = jokes.filter((jokes) => jokes.jokeType === types)
...
app.put("/jokes", (req, res)=>{
  ...
  const replacementJoke = {
    id :id,
    type : req.body.type || currentJoke.jokeTypeype,
    text : req.body.text || currentJoke.jokeText,
  }
  const searchIndex = jokes.findIndex((jokes) => jokes.id === id);
  jokes[searchIndex] = replacementJoke;
...
app.delete("/all", (req, res)=>{
  ...
  if(masterKey ===  userKey){
    ...
    res
    .status(404)
    .json({ errro: `You are not authorised to perform this action`})
  }
})
